...
class User(AbstractBaseUser, IsDeletedModel, PermissionsMixin):
    ...
    # указываем что почта пользоваеться его именем для входа в систему
    USERNAME_FIELD = "email"

    # Обезательные поля для заполнения при создании пользователя
    ...
    def has_perm(self, perm, obj=None):
        """Проверяет, есть ли у пользователя конкретное право доступа (permission)."""
        return True

    def has_module_perms(self, app_label):
        """Проверяет, есть ли у пользователя любые права в указанном приложении (app)."""
        return True
